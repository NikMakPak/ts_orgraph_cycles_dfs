...
class Graph {
    #vertices = new Map<number,Vertex>(); // id,Vertex>
    #edges = new Map<string, Edge>();
    #adjacentList = new Map<number,number[]>(); // vertex id, edges ids
    ...
    addVert(id:number, name:string="") {
        if(!this.vertExist(id)) {
            let vertex = new Vertex(id,name)
            this.vertices.set(id, vertex)
            this.#adjacentList.set(vertex.id, []);
        }
        else throw new Error(`Err: the vertex #${id} is already exist`);
    }

    editVert(id:number, newName:string){
        if(this.vertExist(id)) {
            this.vertices.get(id).rename(newName)
        }
        else throw new Error(`Err: the vertex #${id} is not exist`);
    }
    ...
    editEdge(start:number, end:number,newW:number) { // edit start end
        if(this.edgeExist(`${start}${end}`)) {
            this.#edges.get(`${start}${end}`).reweigh(newW)
        } else throw new Error(`Err: the edge ${start}->${end} is not exist`);
    }

    delEdge(start:number, end:number) { // edit start end
        if(this.edgeExist(`${start}${end}`)) {
            if (this.vertExist(start)) this.vertices.get(start).edges.delete(`${start}${end}`)
    ...
    addEdge(start:number, end:number, weight:number) {
        ...
        } else throw new Error(`Err: the edge ${start}->${end} is already exist or no such vertices exist`);
    }
    ...
    vertExist(id:number):boolean{
        return this.vertices.has(id)
    }

    edgeExist(id:string):boolean{
        return this.#edges.has(id)
    }

    getFirst(id:number):number | undefined {
        if(this.vertExist(id)) {
    ...
    getNext(id:number, i:number):number {
        if(this.vertExist(id)) {
...
function getAllCycles(graph:Graph):object {
    const {vertices, adjacentList} = graph;
    let res: number[][] = []
    let graphCycles:object = {}
    function recursiveVisit(currId:number, path:number[]) {
    ...
    function isUniq(arr:number[]):boolean {
        if (!graphCycles.hasOwnProperty(arr.length)) return true
        let uniq = true
        graphCycles[arr.length].forEach((currArr:number[])=>{
            if (getFormat(currArr).join() == arr.join()) uniq = false
        })
        return uniq
    }
    function getFormat(arr:number[]):number[] {
        return Array.from(new Set(arr)).sort();
    }
    return graphCycles;
}
function getLenCycle(graph:Graph,len:number) {
